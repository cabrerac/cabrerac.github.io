---
author: Christian Cabrera Jojoa
course_code: 26-udenar-big-data
email: chc79@cam.ac.uk
layout: lecture
lecture_code: week-1-hub-es
lecture_date: 06/06/2026
permalink: /teaching/26-udenar-big-data/week-1-hub-es/
position: Assistant Research Professor
session: 1
skip_notebook: true
skip_slides: true
title: "Semana 1 \u2014 hub de enlaces"
visible: false
---

<link rel="stylesheet" href="/assets/css/slides.css">
<link rel="stylesheet" href="/assets/css/lecture-article.css">
**Sábado 6 jun 2026 · 07:00–13:00 (Colombia)** · Lecciones **L1** (Introducción) + **L2** (Ética y gobernanza)

---

### Lecciones y plan del sábado

| Recurso | Enlace |
|---------|--------|
| Lección 1 — Introducción | [l1-introduction](/teaching/26-udenar-big-data/l1-introduction/) |
| Lección 2 — Ética y gobernanza | [l2-ethics-governance](/teaching/26-udenar-big-data/l2-ethics-governance/) |
| Plan de sesión (PDF) | [week-1-session-plan-es.pdf](/assets/documents/26-udenar-big-data/week-1-session-plan-es.pdf) |
| Página del curso | [/teaching/26-udenar-big-data/](/teaching/26-udenar-big-data/) |

---

### Colabs

| Cuaderno | Colab |
|----------|-------|
| L1 individual | [l1-introduction](https://colab.research.google.com/github/cabrerac/cabrerac.github.io/blob/gh-pages/assets/notebooks/26-udenar-big-data/l1-introduction.ipynb) |
| L2 individual | [l2-ethics-governance](https://colab.research.google.com/github/cabrerac/cabrerac.github.io/blob/gh-pages/assets/notebooks/26-udenar-big-data/l2-ethics-governance.ipynb) |
| Semana 1 grupal | [week-1-group](https://colab.research.google.com/github/cabrerac/cabrerac.github.io/blob/gh-pages/assets/notebooks/26-udenar-big-data/week-1-group.ipynb) |

---

### Instalaciónn Local

Para reproducir localmente se pueden installar las dependencias de cada cuaderno utilizando los siguientes archivos:

| Cuaderno | Requisitos |
|----------|--------------------|
| L1 individual | [l1-introduction-requirements.txt](/assets/notebooks/26-udenar-big-data/l1-introduction-requirements.txt) |
| L2 individual | [l2-ethics-governance-requirements.txt](/assets/notebooks/26-udenar-big-data/l2-ethics-governance-requirements.txt) |
| Semana 1 grupal | [week-1-group-requirements.txt](/assets/notebooks/26-udenar-big-data/week-1-group-requirements.txt) |

---

### Plantillas y documentos

| Recurso | Enlace |
|---------|--------|
| Reflexión semana 1 (Word) | [reflection-week-1-template.docx](/assets/documents/26-udenar-big-data/reflection-week-1-template.docx) |
| Requerimientos del proyecto (Word) | [project-requirements-template.docx](/assets/documents/26-udenar-big-data/project-requirements-template.docx) |
| Definición del proyecto (PDF) | [project-definition-big-data.pdf](/assets/documents/26-udenar-big-data/project-definition-big-data.pdf) |

---
